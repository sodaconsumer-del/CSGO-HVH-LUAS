...
local print = function(...)
    ...
    client.color_log(accent.r, accent.g, accent.b, "[pacantech] \0")
...
http.get('https://pacantech.cc/assets/images/logopacantech2.png', function(s, r)
...
local database_key = "pacantech_best_lua_OnG_for_real:configs"
...
local user = _USER_NAME and _USER_NAME or "reezy @deagletus discord"
...
local menu = {
    ...
    enable = groups.main:checkbox("\vpacantech.cc", {ui.get(ref.ui_color)}),
    ...
    home = {
        ...
        welcome = groups.main:label("Welcome to \vpacantech.cc"), --done
    ...
    rage = {
        ...
        disclaimer = groups.main:label("\aFF0F05FFâ  PLAYER MUST NOT DORMANT â "),
    ...
    visuals = {
        ...
        crosshair = groups.main:combobox("crosshair indicators", {"none", "pixel", "pacantech"}), -- done
        ...
        watermark = groups.main:combobox("watermark", {"default", "pacantech", "bottom"}), -- done
    ...
    handles.setupmenu = function(LPH_NO_VIRTUALIZE)
        ...
        for i = 1, #states do
            ...
            if i == 1 then
                ...
                menu.antiaim.builder[i].override = groups.main:checkbox("\v" .. states[i] .. " Â· \rOverride ")
            ...
            menu.antiaim.builder[i].pitch = groups.main:combobox("\v" .. states[i] .. " Â· \rpitch \v" , {"disabled", "down", "up", "zero"})

            menu.antiaim.builder[i].yaw = groups.main:combobox("\v" .. states[i] .. " Â· \ryaw \v ", {"disabled", "backward", "left/right"})

            menu.antiaim.builder[i].yaw_value = groups.main:slider("\n\n\n\n\n\n\n\n",-180, 180, 0)

            menu.antiaim.builder[i].classic1 = groups.main:slider("\v" .. states[i] .. " Â· \r[l|r] \v ",-180, 180, 0)

            menu.antiaim.builder[i].classic2 = groups.main:slider("\n\n\n\n\n\n\n\n\n",-180, 180, 0)

            menu.antiaim.builder[i].modifier = groups.main:combobox("\v" .. states[i] .. " Â· \rmodifier \v ", {"off", "offset", "center", "random", "skitter", "center (skeet)", "offset (skeet)"})

            menu.antiaim.builder[i].mod_slider = groups.main:slider("\n\n\n\n\n\n\n\n\n\n", -180, 180, 0)

            menu.antiaim.builder[i].bodyyaw = groups.main:combobox("\v" .. states[i] .. " Â· \rbodyyaw \v ", {"off", "jitter", "left", "right", "delayed", "break", "jitter (skeet)"})

            menu.antiaim.builder[i].delay = groups.main:slider("\v" .. states[i] .. " Â· \rdelay \v ", 2, 64, 0, true, "t", 1)

            menu.antiaim.builder[i].defensive = groups.main:combobox("\v" .. states[i] .. " Â· \rdefensive \v ", {"-", "on peek", "always on"})

            menu.antiaim.builder[i].defensive_pitch = groups.main:combobox("\v" .. states[i] .. " Â· \rdefensive pitch \v ", {"disabled", "off", "up", "zero", "random", "custom", "switch"})
            ...
            menu.antiaim.builder[i].defensive_yaw = groups.main:combobox("\v" .. states[i] .. " Â· \rdefensive yaw \v ", {"disabled", "static","jitter","sideways", "random", "spin", "move-based"})
            ...
            for j = 1, #phase do
                ...
                menu.antiaim.builder[i].brute[j].override = groups.main:checkbox("\v" .. states[i] .. " Â· \renable\v".. " Â· " .. phase[j])

                menu.antiaim.builder[i].brute[j].pitch = groups.main:combobox("\v" .. states[i] .. " Â· \rpitch\v".. " Â· " .. phase[j], {"disabled", "down", "up", "zero"})

                menu.antiaim.builder[i].brute[j].yaw = groups.main:combobox("\v" .. states[i] .. " Â· \ryaw\v".. " Â· " .. phase[j], {"disabled", "backward", "left/right"})

                menu.antiaim.builder[i].brute[j].yaw_value = groups.main:slider("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",-180, 180, 0)

                menu.antiaim.builder[i].brute[j].classic1 = groups.main:slider("\v" .. states[i] .. " Â· \r[l|r]\v".. " Â· " .. phase[j],-180, 180, 0)

                menu.antiaim.builder[i].brute[j].classic2 = groups.main:slider("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",-180, 180, 0)

                menu.antiaim.builder[i].brute[j].modifier = groups.main:combobox("\v" .. states[i] .. " Â· \rmodifier\v".. " Â· " .. phase[j], {"off", "offset", "center", "random", "skitter"})

                menu.antiaim.builder[i].brute[j].mod_slider = groups.main:slider("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n", -180, 180, 0)

                menu.antiaim.builder[i].brute[j].bodyyaw = groups.main:combobox("\v" .. states[i] .. " Â· \rbodyyaw\v".. " Â· " .. phase[j], {"off", "jitter", "left", "right", "delayed", "break"})

                menu.antiaim.builder[i].brute[j].delay = groups.main:slider("\v" .. states[i] .. " Â· \rdelay\v".. " Â· " .. phase[j], 2, 64, 0, true, "t", 1)

                menu.antiaim.builder[i].brute[j].defensive = groups.main:combobox("\v" .. states[i] .. " Â· \rdefensive\v".. " Â· " .. phase[j], {"-", "on peek", "always on"})

                menu.antiaim.builder[i].brute[j].defensive_pitch = groups.main:combobox("\v" .. states[i] .. " Â· \rdefensive pitch\v".. " Â· " .. phase[j], {"disabled", "off", "up", "zero", "random", "custom", "switch"})
                ...
                menu.antiaim.builder[i].brute[j].defensive_yaw = groups.main:combobox("\v" .. states[i] .. " Â· \rdefensive yaw\v".. " Â· " .. phase[j], {"disabled", "static","jitter","sideways", "random", "spin", "move-based"})
...
local visuals = {} do
    ...
    visuals.indicators = function(self, LPH_NO_VIRTUALIZE)
        ...
        local indx = renderer.measure_text("-", "pacantech")



        if menu.visuals.crosshair:get() == "pixel" then

            renderer.text(screensize.x / 2 + indicator_x - indx / 2, screensize.y / 2 + visuals.positions[offset], 255, 255, 255, 255, "-", 0, tools.gradient_text(accent.r, accent.g, accent.b, 255 * transparency, 0, 0, 0, 255 * transparency, 'pacantech', 2))
        ...
        if menu.visuals.crosshair:get() == "pacantech" then
            ...
            if (fs and fs_key) or antiaim.side1 ~= "reset" then
                ...
                renderer.text(screensize.x/2+scoped_offset, screensize.y/2+37, 255,255,255 ,255 * transparency, "c",nil, tools.gradient_text(accent.r, accent.g, accent.b, 255 * transparency, 255, 255, 255, 255 * transparency, 'pacantech', 0.1))

            end

            renderer.text(screensize.x/2+scoped_offset, screensize.y/2+25, 255,255,255,200 * transparency, "c",nil, math.floor(desync) .. "Â°")
    ...
    visuals.watermark = function(self, LPH_NO_VIRTUALIZE)
        ...
        if menu.visuals.watermark:get() == "bottom" then
            ...
            renderer.text(screen_size_x/2, screen_size_y - 11, 255, 255, 255, 0, "c", nil, tools:text_fade(2, r, g, b, a, " PACANTECH"))

        elseif menu.visuals.watermark:get() == "pacantech" then

            renderer.text(screen_size_x - 55, 10, 255, 255, 255, 0, "c", nil, tools:text_fade(2, r, g, b, a, "pacantech"))
        ...
        elseif menu.visuals.watermark:get() == "default" then
            ...
            -- renderer.text(screen_size_x/2 + 900, screen_size_y/2 - 530, 255, 255, 255, 0, "c", nil, tools:text_fade(2, r, g, b, a, "PACANTECH"))
...
local main = {} do
    ...
    main.phrases = {
        ...
        "ну и чит у тя хуйня шваль",

        "insane resolver",

        "норм переиграл тя бомжа",

        "че делаешь?",

        "Сыр_Ный_Пи_Дор",

        "резик пацантеча тоже прикупи педик ебаный",

        "ahaha",

        "пиздатые аашки только в пацантеч",

        "купишь пацантеч может?",

        "без айди без кфг без луа ваще долбаеб",

        "норм аашки бедолага ебаная",

        "хуем тебе шею сломал",
    ...
    main.killsay_nade = {
        ...
        "взрыв жопы твоей матери",

        "BOMBO",

        "взрыв пизды твоей матери",
...
function InitUtils()
    ...
    Utils.SafeError = function(m_szFunction, m_szError, ret)
        ...
        print('[pacantech](' .. m_szFunction .. ') ' .. m_szError)
